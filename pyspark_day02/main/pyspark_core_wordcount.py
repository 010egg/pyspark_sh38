...
# 2扁平化处理
flatmap_rdd = filter_rdd.flatMap(lambda line: line.strip().split(" ")) # 切割的东西在一个集合里
print(flatmap_rdd.collect())
# 3二元组构建
tuple_rdd = flatmap_rdd.map(lambda word: (word,1))
r_rdd = tuple_rdd.reduceByKey(lambda tmp,item: tmp + item)
r_rdd.foreach(lambda item: print(item))
# r_rdd.saveAsTextFile(path="../datas/output/wc-output1")
